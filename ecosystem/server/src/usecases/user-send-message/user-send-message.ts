...
export class UserSendMessage {
  ...
  async send(messagePack: MessagePack): Promise<Either<InvalidNameError | InvalidUsernameError, Message>> {
    ...
    const saveMessageInRepositoryOrError: SaveMessageResponse = await this.messagesRepository.saveMessage(
      adaptedMessageOrError.value
    );
  ...
  adaptReceivedPackInEntityFormat(
    messagePack: MessagePack
  ): Either<InvalidNameError | InvalidUsernameError | InvalidPasswordError, Message> {
